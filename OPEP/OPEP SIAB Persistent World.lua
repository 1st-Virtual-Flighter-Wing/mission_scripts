...
function SEF_SaveUnitIntermentTable(timeloop, time)
	IntermentMissionStr = IntegratedserializeWithCycles("SIABUnitInterment", SIABUnitInterment)
	writemission(IntermentMissionStr, "SIABUnitInterment.lua")
...
function SEF_SaveStaticIntermentTable(timeloop, time)
	IntermentMissionStrStatic = IntegratedserializeWithCycles("SIABStaticInterment", SIABStaticInterment)
	writemission(IntermentMissionStrStatic, "SIABStaticInterment.lua")
	--trigger.action.outText("Progress Has Been Saved", 15)	
	return time + SaveScheduleUnits
end
...
--////LOAD UNITS
if file_exists("SIABUnitInterment.lua") then
	...
	dofile("SIABUnitInterment.lua")
	
	UnitIntermentTableLength = SEF_GetTableLength(SIABUnitInterment)	
	--trigger.action.outText("Unit Table Length Is "..UnitIntermentTableLength, 15)
		
	for i = 1, UnitIntermentTableLength do
		--trigger.action.outText("Unit Interment Element "..i.." Is "..SIABUnitInterment[i], 15)		
		
		if ( Unit.getByName(SIABUnitInterment[i]) ~= nil ) then
			Unit.getByName(SIABUnitInterment[i]):destroy()
			SEFDeletedUnitCount = SEFDeletedUnitCount + 1
		else
			trigger.action.outText("Unit Interment Element "..i.." Is "..SIABUnitInterment[i].." And Was Not Found", 15)
		end	
	end			
else
	DeadUnitsList = SET_UNIT:New():FilterCoalitions("red"):FilterCategories("ground"):FilterActive(true):FilterStart()
...
--////LOAD STATICS
if file_exists("SIABStaticInterment.lua") then
	
	dofile("SIABStaticInterment.lua")
		
	StaticIntermentTableLength = SEF_GetTableLength(SIABStaticInterment)	
	--trigger.action.outText("Static Table Length Is "..StaticIntermentTableLength, 15)
	
	for i = 1, StaticIntermentTableLength do
		--trigger.action.outText("Static Interment Element "..i.." Is "..SIABStaticInterment[i], 15)
		
		if ( StaticObject.getByName(SIABStaticInterment[i]) ~= nil ) then		
			StaticObject.getByName(SIABStaticInterment[i]):destroy()		
			SEFDeletedStaticCount = SEFDeletedStaticCount + 1
		else
			trigger.action.outText("Static Interment Element "..i.." Is "..SIABStaticInterment[i].." And Was Not Found", 15)
		end	
	end	
else
	SIABStaticInterment = {}
	...
	if ( DEADUNITCOALITION == 1 ) then		
		if ( DEADUNITOBJECTCATEGORY == 1 ) then -- UNIT
			if ( DEADUNITCATEGORY == 2 or DEADUNITCATEGORY == 3 ) then -- GROUND_UNIT or SHIP
				...
				SIABUnitInterment[UnitIntermentTableLength] = DEADUNITNAME			
			else
			end
		elseif ( DEADUNITOBJECTCATEGORY == 3 ) then -- STATIC
			StaticIntermentTableLength = StaticIntermentTableLength + 1			
			SIABStaticInterment[StaticIntermentTableLength] = DEADUNITNAME		
		else
		end		
	else
	end
